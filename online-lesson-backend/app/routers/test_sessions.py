...
def generate_certificate(
    ...
    # Check if user passed (≥75%)
    if session.passed != 1:
        raise HTTPException(
            status_code=403, 
            detail="Sertifikat olish uchun kamida 75% ball to'plash kerak"
        )
    ...
    # "This certifies that"
    c.drawCentredString(width / 2, height - 220, "Ushbu sertifikat quyidagi shaxsga beriladi:")
    ...
    c.setFont("Helvetica", 14)
    c.setFillColor(colors.black)
    c.drawCentredString(width / 2, height - 320, "Yakuniy testni muvaffaqiyatli yakunladi")
    ...
    c.setFont("Helvetica", 14)
    c.setFillColor(colors.black)
    result_text = f"To'g'ri javoblar: {session.correct_answers} / {session.total_questions}"
    c.drawCentredString(width / 2, height - 410, result_text)
    
    score_text = f"Natija: {session.score_percentage}%"
    c.setFont("Helvetica-Bold", 16)
    score_color = "#52c41a" if session.score_percentage >= 70 else "#faad14" if session.score_percentage >= 50 else "#ff4d4f"
